...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, t_emb_dim, up_sample = True, num_heads = 4, num_layers = 1 ,*args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
        self.num_layers = num_layers
...
class UNet(nn.Module):
    def __init__(self, model_configs ,*args: Any, **kwargs: Any) -> None:
        ...
        self.t_emb_dim = model_configs["time_emb_dim"]

        self.down_sample = model_configs["down_sample"]
        self.num_down_layers = model_configs["num_down_layers"]
        self.num_mid_layers = model_configs["num_mid_layers"]
        self.num_up_layers = model_configs["num_up_layers"]
        ...
        for i in range(len(self.down_channels) - 1):
            self.downs.append(
                DownBlock(self.down_channels[i], self.down_channels[i+1], self.t_emb_dim, down_sample=self.down_sample[i], num_layers=self.num_down_layers)
            )

        self.mids = nn.ModuleList([])
        for i in range(len(self.mid_channels) - 1):
            self.mids.append(
                MidBlock(self.mid_channels[i], self.mid_channels[i+1], self.t_emb_dim, num_layers=self.num_mid_layers)
            )
